...
async def update_category(category_id: int, category: CategoryCreate, db: AsyncSession = Depends(get_async_db)):
    ...
    # Обновляем категорию
    update_data = category.model_dump(exclude_unset=True) # Больше для Patch exclude_unset=True
    await db.execute(
    ...
    await db.commit()
    # await db.refresh(db_category) # Если expire_on_commit = False то избыточно
    return db_category
